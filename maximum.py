...
def addToGraphGreedyLinks(g, arr):
    for i in range(0,len(arr),2):
        Graph.add_edge(g, int(arr[i]), int(arr[i + 1]));
        #print(arr[i], arr[i + 1])
...
def simulation(pp, percentage, net, ranking, run, link, lp, greedyArray, *args):
    # print(link)
    s = 1;
    isInfecting = True
    ...
    y = [0,1,2,3,4,5]
    g = Graph(n=len(y), directed=True, vertex_attrs={'name': y})
    ...
    readFileToArray('/Users/apple/Desktop/nets/networks/' + net[0:3] + "_" + str(iter) + ".txt", array)
    #print(link)
    addToGraphGreedyLinks(g, link);
                    ...
                    if notinfected:
                        for k in range(0, numberofneighbors):
                            if (numberofneighbors >= 1):
                                ...
                                #print(x, g.vs[j]['name'], g.vs[notinfected[k]]['name'])
                                if (float(x) <= pp):
            ...
            for i in range(1, 2):
                for n in net:
                    ...
                    for j in range(1, 2):
                        ...
                        myFile = open('syntetic/RESULTS/' + n[0:3] + 'resultMAX.csv', 'a')
                        with myFile:
                            myFields = ['ranking',
                                        'run',
                                        'net',
                    ...
                    for iter in range(1,10):
                        for link in combinationsArray:
                            #print(link)
                            temp2 = simulation(pp, sp, n, rank, i, link, True, greedyArray,iter)
                            myFile = open('syntetic/RESULTS/' + n[0:3] + 'resultMAX.csv', 'a+')
                            with myFile:
                                myFields = ['ranking',
                                            'run',
